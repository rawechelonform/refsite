...
if (isIOSChrome) document.documentElement.classList.add("ios-chrome");
...
let _lineMap = null;              // for hit-testing (non-CE paths)
...
// Enforce on hidden ML input + block rogue submit
(function enforceHiddenFormWhitelist(){
  ...
  mlForm.addEventListener("submit", (ev) => {
    const visibleValue =
      (isIOSChrome && ceEl ? ceEl.textContent : (typedEl?.isContentEditable ? typedEl.textContent : inputEl?.value)) || "";
    if (!isValidEmail(visibleValue)) {
      ev.preventDefault();
      try { mlEmail.setCustomValidity("invalid email."); mlEmail.reportValidity(); } catch(_) {}
...
// ===== NON-CE MIRROR HELPERS (desktop / Android / iOS Chrome mirror output) =====
...
function renderMirror(){
  ...
  if (typedEl.isContentEditable) return;              // Safari uses native CE; no mirror spans
...
// ===== LINE MAP + HIT-TEST (for non-CE paths) =====
...
function indexFromPoint(clientX, clientY){
  ...
  // fallback: nearest char on the closest visual row
...
// ===== RAF (skip for iOS Chrome CE and iOS Safari CE) =====
let _v = null, _s = -1, _e = -1;
function caretRAF(){
  if (!inputEl) { requestAnimationFrame(caretRAF); return; }
  if (isIOSChrome && iosChromeUsingCE) { requestAnimationFrame(caretRAF); return; }
  if (typedEl && typedEl.isContentEditable) { requestAnimationFrame(caretRAF); return; }
...
function ensureIOSChromeCE(){
  ...
  Object.assign(ceEl.style, {
    ...
    textDecoration: "none", WebkitTextDecorationSkip: "none",
    textDecorationColor: "transparent", WebkitTapHighlightColor: "transparent",
  ...
  const paint = () => {
    ...
    typedEl.style.touchAction = "none"; promptEl.style.touchAction = "none";
    const raw = getText(); const sel = getSel();
    if (raw.length === 0) { typedEl.innerHTML = `<span class="cursor-block">&nbsp;</span>`; _lineMap = null; return; }
    if (sel.collapsed) {
      const idx = sel.start;
      const before = htmlAtRange(0, idx, raw);
      const atChar = raw[idx] ? escapeHTML(raw[idx]) : "&nbsp;";
  ...
  const syncFromCE = () => { if (inputEl) inputEl.value = getText().trim(); paint(); };
  ceEl.addEventListener("beforeinput", (e) => {
    const t = e.inputType || ""; if (t === "insertLineBreak" || t === "insertParagraph") e.preventDefault();
  });
  ceEl.addEventListener("input", syncFromCE);
  ceEl.addEventListener("keyup", (e) => { if (e.key === "Enter") { e.preventDefault(); trySubmitEmail(); return; } paint(); });
  ceEl.addEventListener("keydown", (e) => { if (e.key === "Enter") { e.preventDefault(); trySubmitEmail(); return; } });
  ceEl.addEventListener("focus", () => { ceFocused = true; applyKeyboardPadding(); const len = (ceEl.textContent || "").length; setSel(len, len); });
  ceEl.addEventListener("blur",  () => { ceFocused = false; setTimeout(() => { if (!ceFocused) document.body.style.paddingBottom = ""; }, 300); });
  ...
  paint();
  ceEl._setSel = setSel; ceEl._paint = paint;
  iosChromeUsingCE = true;
}

// ===== SAFARI iOS: turn #typed into a real plaintext editor =====
function enableIOSSafariEditor(){
  if (!isIOSSafari || !typedEl) return;

  // Make #typed the editor
  typedEl.contentEditable = "true";
  typedEl.setAttribute("role","textbox");
  typedEl.setAttribute("aria-label","email input");
  typedEl.spellcheck = false;
  typedEl.autocapitalize = "off";
  typedEl.autocorrect = "off";
  // Styling to allow natural selection & wrapping
  Object.assign(typedEl.style, {
    WebkitUserModify: "read-write-plaintext-only",
    ...
    touchAction: "manipulation"   // allow taps/selection; we don't need to block scroll
  });

  // Seed from the hidden input
  typedEl.textContent = (inputEl?.value || "");

  // Keep hidden input in sync for submit
  const syncFromTyped = () => { if (inputEl) inputEl.value = (typedEl.textContent || "").replace(/\n/g, "").trim(); };
  typedEl.addEventListener("input", syncFromTyped);
  ...
  // Focus behavior: focus editor on tap in prompt
  promptEl.addEventListener("touchstart", () => {
    if (submittedUI) return;
    try { typedEl.focus(); } catch(_) {}
  }, { passive: true });

  // Tap to the right of the typed text → caret to end
  const tapRightToEnd = (clientX, clientY) => {
    const tr = typedEl.getBoundingClientRect();
    const H_PAD = 32;
    if (clientX > tr.right + H_PAD) {
      const len = (typedEl.textContent || "").length;
      const range = document.createRange();
      const node = typedEl.firstChild || typedEl;
      try {
        range.setStart(node, Math.min(len, node.length || len));
        range.collapse(true);
        const sel = window.getSelection();
  ...
  typedEl.addEventListener("touchstart", (e) => {
    ...
    if (!t) return;
    // If tap is to the right, jump to end (let Safari manage highlight otherwise)
    if (tapRightToEnd(t.clientX, t.clientY)) e.preventDefault();
  }, { passive: false });
}
...
function bindPrompt(){
  ...
  // Desktop mouse + Android + iOS Chrome mirror interactions remain
  if (!isTouch || (isIOS && !isIOSChrome)) {
    // mouse selection (desktop) — unaffected
    let dragging = false;
    let dragAnchorIdx = null;

    typedEl && typedEl.addEventListener("mousedown", (e) => {
      if (submittedUI || typedEl.isContentEditable) return; // Safari uses native CE
    ...
    window.addEventListener("mousemove", (e) => {
      if (typedEl?.isContentEditable) return;
  ...
  // iOS Chrome: keep previous CE shim behavior (unchanged)
  if (isIOSChrome && window.PointerEvent) {
    bindKeyboardAvoidance();
    // pointer-based selection already implemented above via CE shim helpers
    // (no changes here to keep Chrome behavior stable)
  }

  // Mirror updates (desktop + iOS Safari when NOT using CE — but Safari DOES use CE)
  if (!(isAndroid || isIOSChrome) && !isIOSSafari) {
  ...
  document.addEventListener("keydown", (e) => {
    if (typedEl?.isContentEditable) return; // Safari uses native select
  ...
  // Enter/Go to submit (inputEl path)
  inputEl.addEventListener("keydown", (e) => {
    const isEnter = (e.key === "Enter" || e.key === "Go" || e.keyCode === 13);
    if (isEnter) { e.preventDefault(); trySubmitEmail(); }
  });
...
function showGO(emailText){
  ...
  if (typedEl) {
    ...
    if (typedEl.isContentEditable) typedEl.contentEditable = "false";
  }
...
function trySubmitEmail() {
  ...
  // Collect value from whichever editor is active
  if (isIOSChrome && iosChromeUsingCE && ceEl) {
    inputEl.value = (ceEl.textContent || "").replace(/\r?\n/g, "").trim();
  } else if (typedEl && typedEl.isContentEditable) {
    inputEl.value = (typedEl.textContent || "").replace(/\r?\n/g, "").trim();
  }
  ...
  const doneFail = () => {
    ...
    if (typedEl?.isContentEditable) typedEl.contentEditable = "true";
    if (hintEl) hintEl.textContent = "hmm… couldn’t reach mail server. try again?";
    renderMirror();
  };
  ...
  const iframeTO = setTimeout(() => { mlIframe.removeEventListener("load", onLoad); }, 9000);

  try {
    if (typeof mlForm.requestSubmit === "function") mlForm.requestSubmit(hiddenBtn);
    else hiddenBtn.click();
  } catch (_) {}
...
document.addEventListener("DOMContentLoaded", () => {
  if (isIOSSafari) enableIOSSafariEditor(); // make #typed the actual editor on Safari iOS
  bindPrompt();
  startAvatar();
  if (!isIOSChrome && !isIOSSafari) requestAnimationFrame(caretRAF); // CE paths skip RAF
