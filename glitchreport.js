...
function ensureIOSChromeCE(){
  ...
  const paint = () => {
    ...
    // mirror should wrap like iOS Chrome layout
...
function bindPrompt(){
  ...
  // ==== iOS Safari: Pointer Events path (drag-select + tap-right→end + keyboard avoidance) ====
  ...
  // iOS Chrome pointer shim (with CE)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (isIOSSafari) bindKeyboardAvoidanceIOSSafari();
...
function ensurePromptVisibleIOSSafari(){
  ...
  requestAnimationFrame(() => {
    ...
    const overflow = (tr.bottom + desiredGap) - safeBottom;
    if (overflow > 0) {
      window.scrollTo({ top: Math.max(0, window.scrollY + overflow), behavior: "auto" });
    }
  });
}

function bindKeyboardAvoidanceIOSSafari(){
  if (!isIOSSafari || !window.visualViewport) return;
  iosSafVvBase = window.visualViewport.height;
  const onVV = () => {
    const vv = window.visualViewport;
    const inset = Math.max(0, Math.round(iosSafVvBase - vv.height));
    document.body.style.paddingBottom = inset > 0 ? (inset + 24) + "px" : "";
    ensurePromptVisibleIOSSafari();
  };
  window.visualViewport.addEventListener("resize", onVV);
  window.visualViewport.addEventListener("scroll", onVV);
  inputEl.addEventListener("focus", onVV);
}
