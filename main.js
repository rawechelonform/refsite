(() => {
  ...
  // ⬇️ Move caption inside the image wrapper BEFORE any rendering happens
  ...
  (async () => {
    try {
      const rows = parseCSV(await (await fetch(SHEET_CSV_URL, { cache: "no-cache" })).text());
      ...
      IMAGES = [
        { src: "homeroom/sadgirls/girl1.png", thumb: null, title: "Girl 1", meta: { title: "Girl 1", year: "", medium: "", size: "" } }
      ];
      render();
    }
  })();
  ...
  // ==== Carousel wiring (kept from your original) ====
  function setStage(i) {
    ...
    img.onerror = () => { stageImg.alt = "Image failed to load"; };
    img.src = item.src;
